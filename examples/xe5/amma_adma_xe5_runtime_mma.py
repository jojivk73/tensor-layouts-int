...
"""XE5 runtime (shapeless) AMMA GEMM

Functional port of the sycl-tla example examples/cute/tutorial/xe5/amma_adma_xe5_runtime_mma.cpp, runnable on the CPU.
Shapeless async_gmma: the MMA shape comes from a runtime control word.

Run:  python examples/xe5/amma_adma_xe5_runtime_mma.py
Each section below is one operation; the same content is in amma_adma_xe5_runtime_mma.ipynb as cells."""
...
# ----------------------------------------------------------------------
# Step — runtime-shaped MMA
# ----------------------------------------------------------------------
# Xe5 runtime/shapeless MMA: the MMA shape comes from a control word at
# runtime, not the atom type. We pick the shape dynamically, then run it.
mma_ctrl = {"M": M, "N": N, "K": K}          # shape known only at runtime
print("runtime MMAControl:", mma_ctrl)
S = A[:mma_ctrl["M"], :mma_ctrl["K"]] @ B[:mma_ctrl["N"], :mma_ctrl["K"]].T
print("S shape from control:", S.shape, "\n", S[:3, :6])
...
# Recap
# ----------------------------------------------------------------------
print('Recap: load → runtime-shaped MMA → store.')
