...
"""XE5 MX scale-factor block-scaled GEMM

Functional port of the sycl-tla example examples/cute/tutorial/xe5/xe5_mx_sf_blockscaled_gemm.cpp, runnable on the CPU.
MX scale factors (fp4/fp6) with a Type-III SF descriptor.

Run:  python examples/xe5/xe5_mx_sf_blockscaled_gemm.py
Each section below is one operation; the same content is in xe5_mx_sf_blockscaled_gemm.ipynb as cells."""
...
# ----------------------------------------------------------------------
# Step — MX scale-factor MMA
# ----------------------------------------------------------------------
# Xe5 MX scale-factor GEMM: fp4/fp6 mantissas with a Type-III scale-factor
# descriptor. Each K-block of 8 shares a power-of-two scale.
BK = 8
sfA = (2.0 ** rng.integers(-2, 3, size=(M, K // BK))).astype(np.float32)
sfB = (2.0 ** rng.integers(-2, 3, size=(N, K // BK))).astype(np.float32)
S = (A * np.repeat(sfA, BK, axis=1)) @ (B * np.repeat(sfB, BK, axis=1)).T
print("MX block size:", BK, " example scales A[0]:", sfA[0])
print("scaled S[:3,:6]:\n", S[:3, :6])
...
# Recap
# ----------------------------------------------------------------------
print('Recap: load → MX-scaled MMA → store.')
