...
Full-execution-flow functional port of the sycl-tla example examples/cute/tutorial/xe5/xe5_mx_sf_blockscaled_gemm.cpp.
MX scale factors (fp4/fp6), Type-III SF descriptor.
...
atom = make_xe4_amma_atom("f32", DT, DT, "f32", BLK_M, BLK_N, BLK_K, tracking="AB", block_scaled=True)
slmA = make_slm_layout_elem(DBITS, BLK_M, BLK_K); slmB = make_slm_layout_elem(DBITS, BLK_N, BLK_K)
BK = BLK_K                                            # one scale factor per K-tile
sfA = rng.uniform(0.5, 2.0, size=(M, K // BK)).astype(np.float32)
sfB = rng.uniform(0.5, 2.0, size=(N, K // BK)).astype(np.float32)
print("block-scaled MMA atom:", atom.name, " SF block (K):", BK)
show_mat("scale factors sfA", sfA)
