...
"""XE5 dual-group AMMA GEMM (cluster)

Full-execution-flow functional port of the sycl-tla example examples/cute/tutorial/xe5/amma_dual_mma_xe5.cpp.
Dual-group MMA (lane pair splits M), full pipeline.

Run:  python examples/xe5/amma_dual_mma_xe5.py"""
...
atom = make_xe5_amma_atom("XE5_AMMA_AB_CLUSTER_DUAL", "amma.atmm.btmm (dual)",
                          "f32", DT, DT, "f32", BLK_M, BLK_N, BLK_K, dual=True, block_scaled=False)
slmA = make_slm_layout_elem(DBITS, BLK_M, BLK_K)
slmB = make_slm_layout_elem(DBITS, BLK_N, BLK_K)
print("MMA atom :", atom.name, " ThrID =", size(atom.thr_id), "(dual-group lane pair)")
