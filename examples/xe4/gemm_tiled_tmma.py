...
Functional port of the sycl-tla example examples/cute/tutorial/xe4/gemm_tiled_tmma.cpp, runnable on the CPU.
The **TMM** register MMA: M fixed at 32, lane i owns row i.

Run:  python examples/xe4/gemm_tiled_tmma.py
Each section below is one operation; the same content is in gemm_tiled_tmma.ipynb as cells."""
...
# ----------------------------------------------------------------------
# Step — TMM register ownership + MMA
# ----------------------------------------------------------------------
# TMM register ownership: the register MMA fixes M=32 and lane i owns row i of
# A and of the accumulator C (A/C layout = (M,K):(1,M)).
Areg = Layout((M, K), (1, M))
print("A register layout (lane, value) -> element (M-major):")
show_layout(Areg, M, K, rl="lane", cl="v")
S = A @ B.T
print("\nlane 0 owns C row 0:", S[0, :6], "...")
