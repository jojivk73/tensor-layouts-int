...
Full-execution-flow functional port of the sycl-tla example examples/cute/tutorial/xe4/gemm_ldsm_warp_row_epilogue_xe4.cpp.
Epilogue via an LDSM warp-row gather.
...
# ----------------------------------------------------------------------
# LDSM warp-row epilogue
# ----------------------------------------------------------------------
# LDSM warp-row epilogue: gather D one row per subgroup via the LDSM layout,
# then store. Show the lane assignment for row 0.
from tensor_layouts.atoms_xe4 import make_xe4_ldstm
ldsm = make_xe4_ldstm("LDSM", VS=8, s=DT)
print("LDSM epilogue atom:", ldsm.name, " ThrID =", size(ldsm.thr_id))
warp = {t: D[0, t] for t in range(min(N, 32))}
show_mat("D row 0 across lanes", np.array([warp[t] for t in range(min(N, 32))]).reshape(1, -1))
check("row gathered intact", [warp[t] for t in range(min(N, 32))] == list(D[0, :min(N, 32)]))
