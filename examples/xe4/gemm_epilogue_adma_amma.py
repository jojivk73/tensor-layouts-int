...
"""XE4 fused ADMA+AMMA epilogue

Full-execution-flow functional port of the sycl-tla example examples/cute/tutorial/xe4/gemm_epilogue_adma_amma.cpp.
Fused SiLU + C-multiply epilogue (D-barrier tracked).

Run:  python examples/xe4/gemm_epilogue_adma_amma.py"""
...
# Epilogue
# ----------------------------------------------------------------------
# Fused activation epilogue (XE4_AMMA_D): D = SiLU(alpha*acc) * C.
lin = alpha * acc
silu = lin / (1.0 + np.exp(-lin))
D = silu * C
show_mat("SiLU(alpha*acc)", silu)
show_mat("D = SiLU(alpha*acc) * C", D)
...
# ----------------------------------------------------------------------
lin = alpha * (A @ B.T)
D_ref = (lin / (1.0 + np.exp(-lin))) * C
err = np.abs(D - D_ref).max()
print("max abs error vs SiLU reference:", err)
check("fused-epilogue verification", err < 1e-3)
