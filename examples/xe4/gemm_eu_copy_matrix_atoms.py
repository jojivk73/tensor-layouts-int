...
Full-execution-flow functional port of the sycl-tla example examples/cute/tutorial/xe4/gemm_eu_copy_matrix_atoms.cpp.
Operands staged by EU-pipeline copies.
...
# ----------------------------------------------------------------------
# EU-copy staging: XE4_EU_COPY_G2S moves each K-tile from a linear (row-major)
# global tile into swizzled SLM on the EU compute pipeline (vs a DMA unit).
linear = Layout((BLK_M, BLK_K), (BLK_K, 1))
print("linear source (m,k)->offset:")
show_layout(linear, BLK_M, BLK_K)
print("swizzled SLM (m,k)->offset:")
show_layout(slmA, BLK_M, BLK_K)
_probe = load_to_slm(a_kt[0], slmA)
check("EU-copy linear->SLM round-trip", np.array_equal(read_slm(_probe, slmA, BLK_M, BLK_K), a_kt[0]))
