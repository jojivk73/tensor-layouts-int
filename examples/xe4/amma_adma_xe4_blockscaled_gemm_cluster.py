...
"""XE4 block-scaled GEMM (cluster)

Functional port of the sycl-tla example examples/cute/tutorial/xe4/amma_adma_xe4_blockscaled_gemm_cluster.cpp, runnable on the CPU.
Block-scaled GEMM with cluster multicast of B.

Run:  python examples/xe4/amma_adma_xe4_blockscaled_gemm_cluster.py
Each section below is one operation; the same content is in amma_adma_xe4_blockscaled_gemm_cluster.ipynb as cells."""
...
# ----------------------------------------------------------------------
# Step — cluster multicast of B
# ----------------------------------------------------------------------
# Cluster multicast: XE4_ADMA_LOAD_MULTICAST broadcasts the B operand to
# every CTA in the cluster (here 2). Each CTA sees the identical B tile.
CLUSTER = 2
b_per_cta = [B.copy() for _ in range(CLUSTER)]
print("B tile corner (identical on every CTA):\n", B[:2, :6], "...")
print("B broadcast to", CLUSTER, "CTAs; all identical:",
      all(np.array_equal(b_per_cta[0], b) for b in b_per_cta))
...
# Recap
# ----------------------------------------------------------------------
print('Recap: multicast → scaled MMA → store.')
